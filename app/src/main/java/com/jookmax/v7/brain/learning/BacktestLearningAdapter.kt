 ...
 *
 * Note:
 * BacktestResult currently does not expose the full market-analysis
 * fields required by LearningExperience, so unavailable values are
 * explicitly represented rather than inferred from the backtest result.
 */
@Singleton
class BacktestLearningAdapter @Inject constructor(
    ...
    fun learnFromBacktest(
        ...
        val success = result.netProfit > 0.0

        val confidence = result.winRate
